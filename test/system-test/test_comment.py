...
class SystemCommentTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # 创建一个测试帖子
        post = PostModel(title='Test Post', content='This is a test post.', post_type='general', author_id=user.id)
        db.session.add(post)
        db.session.commit()
    ...
    def test_create_comment(self):
        driver = self.driver
        driver.get(self.base_url + '/posts/1')  # 假设帖子 ID 为 1

        # 填写评论表单
        driver.find_element(By.NAME, 'content').send_keys('This is a test comment.')
